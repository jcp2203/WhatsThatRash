...
%% process averages of all images
        ...
        subplot(2,3,1)
        hold on
        histfit(mean_l{1}(:)./ceil(max(mean_l{1}(:))),5,'beta') %for beta distribution fit
        histfit(mean_a{1}(:)./ceil(max(mean_a{1}(:))),5,'beta') %for beta distribution fit
        histfit(mean_b{1}(:)./ceil(max(mean_b{1}(:))),5,'beta') %for beta distribution fit
        title('Atopic Dermatitis');
        legend('Intensity','Intensity curve','Green-Red', 'G-R curve','Blue-Yellow', 'B-Y curve');
        hold off
        
        subplot(2,3,2)
        hold on
        histfit(mean_l{2}(:)./ceil(max(mean_l{2}(:))),5,'beta') %for beta distribution fit
        histfit(mean_a{2}(:)./ceil(max(mean_a{2}(:))),5,'beta') %for beta distribution fit
        histfit(mean_b{2}(:)./ceil(max(mean_b{2}(:))),5,'beta') %for beta distribution fit
        title('Christmas Rash');
        hold off
        
        subplot(2,3,3)
        hold on
        histfit(mean_l{3}(:)./ceil(max(mean_l{3}(:))),5,'beta') %for beta distribution fit
        histfit(mean_a{3}(:)./ceil(max(mean_a{3}(:))),5,'beta') %for beta distribution fit
        histfit(mean_b{3}(:)./ceil(max(mean_b{3}(:))),5,'beta') %for beta distribution fit        
        title('Contact Dermatitis');
        hold off
        
        subplot(2,3,4)
        hold on
        histfit(mean_l{4}(:)./ceil(max(mean_l{4}(:))),5,'beta') %for beta distribution fit
        histfit(mean_a{4}(:)./ceil(max(mean_a{4}(:))),5,'beta') %for beta distribution fit
        histfit(mean_b{4}(:)./ceil(max(mean_b{4}(:))),5,'beta') %for beta distribution fit
        title('Malignant Melanoma');
        hold off
        
        subplot(2,3,5)
        hold on
        histfit(mean_l{5}(:)./ceil(max(mean_l{5}(:))),5,'beta') %for beta distribution fit
        histfit(mean_l{5}(:)./ceil(max(mean_l{5}(:))),5,'beta') %for beta distribution fit
        histfit(mean_l{5}(:)./ceil(max(mean_l{5}(:))),5,'beta') %for beta distribution fit
        title('Psoriasis');
        hold off
        ...
        subplot(2,3,6)
        hold on
        histfit(mean_l{6}(:)./ceil(max(mean_l{6}(:))),5,'beta') %for beta distribution fit
        histfit(mean_a{6}(:)./ceil(max(mean_a{6}(:))),5,'beta') %for beta distribution fit
        histfit(mean_b{6}(:)./ceil(max(mean_b{6}(:))),5,'beta') %for beta distribution fit        
        title('Ringworm');
        hold off
        ...
        axis image; % Make sure image is not artificially stretched because of screen's aspect ratio.
        subplot(2,3,i)
        histfit(std_l./ceil(max(std_l)),5,'beta')
        title('std_dev_L')
    %}
